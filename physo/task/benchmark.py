...
def dummy_epoch(X, y, run_config):
    ...
    for step in range(batch.max_time_step):
        ...
        # ---- Prior ----
        prior = torch.tensor(
            batch.prior().astype(np.float32)
        )  # (batch_size, n_choices,)                                                                                 # (batch_size, obs_size,)

        # ---- Dummy cell output ----
        probs = torch.tensor(np.random.rand(batch.batch_size,
                                            batch.library.n_choices).astype(np.float32))  # (batch_size, n_choices,)
...
def sanity_check(X, y, run_config, candidate_wrapper=None, target_program_str=None, expected_ideal_reward=1.):
    ...
    def cell_reseter():
        ...
        cell = rnn.Cell(input_size=input_size, output_size=output_size, **run_config["cell_config"])
    ...
    if target_program_str is not None:
        ...
        target_program = program.Program(tokens=[batch.library.lib_name_to_token[name] for name in target_program_str],
                                         ...
                                         is_physical=True,
                                         free_const_values=torch.tensor(batch.library.free_constants_init_val).to(
                                             batch.dataset.detected_device),
                                         candidate_wrapper=candidate_wrapper)
        ...
        if batch.library.n_free_const > 0:
            ...
            history = target_program.optimize_constants(
                X=batch.dataset.X,
        ...
        eps = 1e-5
        assert (expected_ideal_reward -
                ideal_reward) <= 2 * eps, 'Ideal reward should be >= %f +/- %f ' % (expected_ideal_reward, eps)
