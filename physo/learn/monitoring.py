...
class RunVisualiser:
    ...
    def __init__(self,
        ...
        if save_path is not None:
            self.save_path_log = ''.join(
                save_path.split('.')[:-1]) + "_data.csv"  # save_path with extension replaced by '_data.csv'
            self.save_path_pareto = ''.join(
                save_path.split('.')[:-1]) + "_pareto.csv"  # save_path with extension replaced by '_pareto.csv'
            self.save_path_pareto_fig = ''.join(
                save_path.split('.')[:-1]) + "_pareto.pdf"  # save_path with extension replaced by '_pareto.pdf'
    ...
    def update_plot(self,):
        ...
        curr_ax = self.ax0
        curr_ax.clear()
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.mean_R_history,
                     'b',
                     linestyle='solid',
                     alpha=0.6,
                     label="Mean")
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.mean_R_train_history,
                     'r',
                     linestyle='solid',
                     alpha=0.6,
                     label="Mean train")
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.overall_max_R_history,
                     'k',
                     linestyle='solid',
                     alpha=1.0,
                     label="Overall Best")
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.max_R_history,
                     color='orange',
                     linestyle='solid',
                     alpha=0.6,
                     label="Best of epoch")
        ...
        for i in range(fading_plot_ncurves + 1):
            ...
            kde = KernelDensity(kernel="gaussian", bandwidth=fading_plot_kde_bandwidth).fit(
                run_logger.R_history_train_array[plot_epoch][:, np.newaxis])
        ...
        curr_ax.clear()
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.best_prog_complexity_history,
                     'orange',
                     linestyle='solid',
                     label="Best of epoch")
        curr_ax.plot(run_logger.epochs_history,
                     run_logger.mean_complexity_history,
                     'b',
                     linestyle='solid',
                     label="Mean")
        ...
        for i in range(fading_plot_ncurves + 1):
            ...
            if distrib_data.shape[0] > 0:
                ...
                kde = KernelDensity(kernel="gaussian",
                                    bandwidth=fading_plot_kde_bandwidth).fit(distrib_data[:, np.newaxis])
                dens = 10**kde.score_samples(bins_dens[:, np.newaxis])
                # Plot
                curr_ax.plot(bins_dens, dens, alpha=alpha, linewidth=0.5, c=cmap(prog))
        # Colorbar
