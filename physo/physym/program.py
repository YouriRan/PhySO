...
class VectPrograms:
    ...
    def __init__(self, batch_size, max_time_step, library, candidate_wrapper=None):
        ...
        self.tokens = Tok.VectTokens(shape=self.shape,
                                     invalid_token_idx=self.invalid_idx)  # (batch_size, max_time_step,)
        ...
        self.units_analysis_cases = np.full(shape=self.shape,
                                            fill_value=phy.UNITS_ANALYSIS_NOT_PERFORMED_CASE_CODE,
    ...
    def append(self, new_tokens_idx, forbid_inconsistent_units=False):
        ...
        assert (new_tokens_idx.dtype == int or
                new_tokens_idx.dtype == np.dtype("int64")), "Arg new_tokens_idx must be a numpy array of dtype = int"
        ...
        mask_necessary_units_analysis_performed = np.logical_or.reduce((~(self.units_analysis_cases[
            :,
            self.curr_step,
        ] == phy.UNITS_ANALYSIS_NOT_PERFORMED_CASE_CODE), self.is_complete))
        ...
        # to new tokens' units.
        self.set_units(
            coords_dest=self.coords_of_step(
                step=self.curr_step)[:, mask_is_constraining_new_tokens],  # (2, n_constraining_new_tokens,) of int
            new_is_constraining_phy_units=np.full(n_constraining_new_tokens,
                                                  True),  # (n_constraining_new_tokens,) of bool
            new_phy_units=units_from_new_tokens[
                mask_is_constraining_new_tokens],  # (n_constraining_new_tokens, Tok.UNITS_VECTOR_SIZE) of float
        )
        ...
        self.is_complete[np.logical_and(self.is_complete == False,
                                        self.n_dummies == 0)] = True  # (bool_array.sum(),) of int
        ...
        mask_legacy_tokens_noshift = np.logical_and.reduce((  # (batch_size, max_time_step,) of bool
            ...
            self.tokens.pos
            < self.tile_batch_vect(n_legacy_dummies) + self.curr_step,  # (batch_size, max_time_step,) of bool
        ))
        ...
        legacy_shift = n_new_dummies[  # (n_legacy_dummies_total,) of int
            coords_legacy_tokens_noshift[
                0]  # coords of legacy tokens in batch dim      # (n_legacy_dummies_total,) of int
        ]
        # Legacy tokens shift
        coords_legacy_tokens_shifted = np.stack(
            (  # (2, n_legacy_dummies_total,) of int
        ...
        mask_new_dummies = np.logical_and.reduce((  # (batch_size, max_time_step,) of bool
            self.tokens.pos >= self.curr_step,  # (batch_size, max_time_step,) of bool
            self.tokens.pos
            < self.tile_batch_vect(n_new_dummies) + self.curr_step,  # (batch_size, max_time_step,) of bool
        ))
        ...
        self.set_parent(
            coords_dest=coords_new_dummies,
            has_mask=True,
            pos_val=self.curr_step - 1,
        )
        ...
        self.tokens.has_siblings_mask[tuple(
            coords_new_dummies_w_siblings_0)] = True  # (n_new_dummies_w_sibling_0,) of bool
        self.tokens.has_siblings_mask[tuple(
            coords_new_dummies_w_siblings_1)] = True  # (n_new_dummies_w_sibling_1,) of bool
        ...
        coords_new_dummies_parents = np.stack(
            (  # (2, n_new_dummies_total,) of int
        ...
        self.tokens.ancestors_pos[tuple(coords_new_dummies)] = self.tokens.ancestors_pos[tuple(
            coords_new_dummies_parents)]
        ...
        pos_lonely_children = np.stack(
            (  # (n_new_tokens_w_lonely_child, 1,) of int
        ...
        # Setting children
        self.set_children(
            coords_dest=tuple(coords_new_tokens_w_lonely_child),  # (2, n_new_tokens_w_lonely_child,) of int
        ...
        pos_double_children = np.stack(
            (  # (n_new_tokens_w_two_children, 2,) of int
        ...
        # Setting children
        self.set_children(
            coords_dest=tuple(coords_new_tokens_w_two_children),  # (2, n_new_tokens_w_two_children,) of int
    ...
    def assign_required_units(
        self,
        step=None,
        ignore_unphysical=True,
    ):
        """
    ...
    def get_siblings(self, coords):
        ...
        siblings_coords = np.concatenate(
    ...
    def get_children(self, coords):
        ...
        children_coords = np.concatenate(
            (  # (1 + Tok.MAX_NB_CHILDREN, ?) of int
                coords[0][:, np.newaxis],  # same batch coords   # batch dim coord  # (?, 1) of int
    ...
    def get_ancestors(self, coords):
        ...
        ancestors_coords = np.concatenate(
            (  # (1 + max_time_step, ?) of int
                coords[0][:, np.newaxis],  # same batch coords   # batch dim coord  # (?, 1) of int
    ...
    def get_property_of_relative(self, coords, relative, attr):
        ...
        assert relative in supported_relationships, "%s not supported, use one of %s" % (relative,
        ...
        attribute = np.full(
            shape=attribute_shape, fill_value=attribute_filler_value, dtype=attribute_type
        )  # (?, (+ depending on attr))                                                                                    # (?,)
        ...
        attribute[is_meaningful] = self.tokens.__getattribute__(
            "%s" % attr)[tuple(coords_existing_relatives)]  # (n_meaningful,)
    ...
    def move_dummies(self, coords_src, coords_dest, do_update_relationships_pos=True, do_fill_with_void=True):
        ...
        self.tokens.is_constraining_phy_units[tuple(coords_dest)] = self.tokens.is_constraining_phy_units[tuple(
            coords_src)]
        ...
        def update_relationships_pos_of_moved_tokens(coords_b_move, coords_a_move):
            ...
            coords_valid_parent = np.stack(
                ...
                axis=0)[:, mask_has_parent]  # where has_parent_mask is True (ie. pos are valid)
            ...
            pos_valid_sibling = self.tokens.siblings_pos[tuple(coords_a_move)][
                mask_has_sibling, :]  # (?1, Tok.MAX_NB_SIBLINGS) of int

            # Coords of siblings when available
            coords_valid_sibling = np.stack(
                (  # (2, ?1,) of int
            ...
            pos_sibling_sibling = self.tokens.siblings_pos[tuple(
                coords_valid_sibling)]  # (?1, Tok.MAX_NB_SIBLINGS) of int
            ...
            # Update matrix of properties
            self.tokens.siblings_pos[tuple(
                coords_valid_sibling)] = pos_sibling_sibling  # (?1, Tok.MAX_NB_SIBLINGS) of int
    ...
    def batch_optimize_constants(self,
                                 X,
                                 y_target,
    ...
    def get_tree_graph(
        ...
        superparent_pos = "--"
        add_token_node(name=superparent.name, pos=superparent_pos, units=superparent.phy_units, color=constraint_color)
