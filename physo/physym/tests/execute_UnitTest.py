...
class ExecuteProgramTest(unittest.TestCase):
    ...
    def test_ExecuteProgram(self):
        ...
        # PROGRAM
        test_program_str = [
            "mul", "mul", "M", "n2", "c", "sub", "inv", "sqrt", "sub", "1", "div", "n2", "v", "n2", "c", "cos", "div",
            "sub", "1", "div", "v", "c", "div", "div", "x", "t", "c"
        ]
        ...
        for _ in range(N):
            res = Exec.ExecuteProgram(
                input_var_data=data,
                program_tokens=test_program,
            )
        ...
        t0 = time.perf_counter()
        for _ in range(N):
            expected_res = M * (c**2) * (1. / torch.sqrt(1. - (v**2) / (c**2)) - torch.cos(
                (1. - (v / c)) / ((x / t) / c)))
        t1 = time.perf_counter()
        print("\nExecuteProgram time (wo tokens) = %.3f ms" % ((t1 - t0) * 1e3 / N))

        # TEST
        works_bool = np.array_equal(
            data_conversion_inv(res.cpu()),
            data_conversion_inv(expected_res.cpu()),
        )
    ...
    def test_A_ParallelizedExeReduceGather(self):
        ...
        my_lib = Lib.Library(args_make_tokens=args_make_tokens, superparent_units=[0, 0, 0], superparent_name="y")
