...
class LibraryTest(unittest.TestCase):
    ...
    def test_library_creation_and_types(self):
        ...
        custom_tokens = [
            ...
            Tok.Token(name='x1', sympy_repr='x1', arity=0, complexity=0, var_type=1, function=None, var_id=1),
            Tok.Token(name='add', sympy_repr='add', arity=2, complexity=0, var_type=0, function=np.add, var_id=None),
            Tok.Token(name='cos', sympy_repr='cos', arity=1, complexity=0, var_type=0, function=np.cos, var_id=None),
            Tok.Token(
                name='pi',
                sympy_repr='pi',
                arity=0,
        ...
        bool_works = np.array_equal(my_lib.superparent.phy_units,
                                    np.full(Tok.UNITS_VECTOR_SIZE, np.NAN),
                                    equal_nan=True)
        self.assertEqual(bool_works, True)
    ...
    def test_some_units_not_provided_warning(self):
        ...
        x0 = Tok.Token(name='x0',
                       ...
                       is_constraining_phy_units=True,
                       phy_units=[1., 0., 0., 0., 0., 0., 0.])
        x1 = Tok.Token(name='x1', sympy_repr='x1', arity=0, complexity=0, var_type=1, function=None, var_id=1)
        add = Tok.Token(name='add', sympy_repr='add', arity=2, complexity=0, var_type=0, function=np.add, var_id=None)
        cos = Tok.Token(name='cos', sympy_repr='cos', arity=1, complexity=0, var_type=0, function=np.cos, var_id=None)
        pi = Tok.Token(name='pi',
                       sympy_repr='pi',
                       arity=0,
                       complexity=0,
                       var_type=0,
                       function=lambda const=np.pi: const,
                       var_id=None)
        c0 = Tok.Token(name='c0', sympy_repr='c0', arity=0, complexity=0, var_type=2, init_val=1., var_id=0)
        args_make_tokens = {
            # operations
            "op_names": ["mul", "neg", "inv", "sin"],
            "use_protected_ops": False,
        ...
        with self.assertWarns(Warning):
            my_lib = Lib.Library(
                custom_tokens=[c0, add],
                args_make_tokens=None,
            )
        with self.assertWarns(Warning):
            my_lib = Lib.Library(
                custom_tokens=[x0, x1, add],
                args_make_tokens=None,
            )
        with self.assertWarns(Warning):
            my_lib = Lib.Library(
        ...
        with self.assertWarns(Warning):
            ...
            my_lib.append_tokens_from_names(args_make_tokens=args_make_tokens)
        with self.assertWarns(Warning):
            my_lib = Lib.Library(custom_tokens=[x0],
                                 args_make_tokens=None,
                                 superparent_units=None,
                                 superparent_name="v")
        with self.assertWarns(Warning):
            my_lib = Lib.Library(custom_tokens=[x0, x1, add],
                                 args_make_tokens=args_make_tokens,
                                 superparent_units=None,
                                 superparent_name="v")
