...
class BatchTest(unittest.TestCase):
    ...
    def test_dummy_epoch(self):
        ...
        # --- PRIORS ---
        priors_config = [("UniformArityPrior", None), ("HardLengthPrior", {
            "min_length": 1,
            "max_length": 5,
        }), ("PhysicalUnitsPrior", {
        ...
        for step in range(max_time_step):
            ...
            observations = torch.tensor(my_batch.get_obs().astype(np.float32),
                                        requires_grad=False)  # (batch_size, 3*n_choices+1)
            # Dummy model output
            probs = torch.tensor(np.random.rand(my_batch.batch_size, my_batch.library.n_choices).astype(
                np.float32))  # (batch_size, n_choices,)
            # Actions
    ...
    def test_dummy_epoch_free_consts(self):
        ...
        eps = 1e-5
        n_solutions = (np.abs(1 - R) < eps).sum()
        print("Dummy epoch time (w free const) = %f ms (found %i/%i candidates with R = 1 +/- %f)" %
              ((t1 - t0) * 1e3, n_solutions, batch_size, eps))
    ...
    def test_dummy_epoch_free_consts_and_duplicate_elimination(self):
        ...
        my_batch = batch.Batch(
            ...
            batch_size=batch_size,
            max_time_step=max_time_step,
            rewards_computer=reward.make_RewardsComputer(
