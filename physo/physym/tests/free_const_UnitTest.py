...
class FreeConstUtilsTest(unittest.TestCase):
    ...
    def test_optimization_process(self):
        ...
        my_lib = Lib.Library(args_make_tokens=args_make_tokens, superparent_units=[2, -2, 0], superparent_name="E")

        # TEST PROGRAMS
        test_programs_idx = []
        test_prog_str_0 = [
            "add", "mul", "mul", "const1", "c0", "n2", "v", "mul", "n2", "vc", "log", "div", "n2", "r", "n2", "rc"
        ]
        ...
        y_target0 = Exec.ExecuteProgram(
            input_var_data=X,
            free_const_values=ideal_const0,
            program_tokens=prog0.tokens,
        )
        y_target1 = Exec.ExecuteProgram(
            input_var_data=X,
            free_const_values=ideal_const1,
            program_tokens=prog1.tokens,
        )
