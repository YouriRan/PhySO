...
def test_one_token(tester, token):
    ...
    if token.arity == 2:
        ...
        tester.assertEqual(len(data_conversion_inv(token(*torch.stack((data0, data1))))),
                           n_data)  # *[np. array    , np.array]
        # large float , large float
        # expecting length = 1 or n_data to be able to compute afterwards
        out_len = np.shape(np.atleast_1d(data_conversion_inv(token(large, large))))
    ...
    if token.arity == 1:
        ...
        # large float
        # expecting length = 1 or n_data to be able to compute afterwards
        out_len = np.shape(np.atleast_1d(data_conversion_inv(token(large))))
        tester.assertEqual(out_len == n_data or out_len == (1,), True)
    # Zero-arity
    if token.arity == 0:
        out_len = np.shape(np.atleast_1d(data_conversion_inv(token())))
...
class FuncTest(unittest.TestCase):
    ...
    def test_protected_unprotected_same_attributes(self):
        unprotected_tokens = Func.make_tokens(
            op_names="all",
            use_protected_ops=False,
        )
        protected_tokens = Func.make_tokens(
            op_names="all",
            use_protected_ops=True,
        )
    ...
    def test_data_pointers_work(self):
        ...
        bool = np.array_equal(
            data_conversion_inv(my_tokens_dict["mul"](my_tokens_dict["data0"](), my_tokens_dict["data1"]())),
    ...
    def test_behavior_contain_different_ops(self):
        ...
        for name in unprotected_tokens_names + protected_tokens_names:
            ...
            for _, behavior in Func.OP_UNIT_BEHAVIORS_DICT.items():
                if (name in behavior.op_names):
                    count += 1
            if count > 1:
                self.fail("Token named %s appears in more than one behavior." % (name))
