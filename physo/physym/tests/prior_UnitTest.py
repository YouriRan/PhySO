...
class PriorTest(unittest.TestCase):
    ...
    def test_HardLengthPrior(self):
        ...
        # ------- TEST CREATION -------
        try:
            my_prior = Prior.HardLengthPrior(library=my_lib,
        ...
        for step in range(n_steps):
            ...
            for i_prog in range(n_progs):
                ...
                works_bool = (prior[i_prog][my_lib.get_choosable_prop("arity") < 1]).all() == (
                    step > pos_end_min_arity_is_1[i_prog])
                self.assertTrue(works_bool, msg="Test case prog %i failed at step = %i " % (i_prog, step))
        ...
        for step in range(n_steps):
            ...
            if step < 2:
                ...
                self.assertTrue(works_bool,
                                msg="prog_1_arity_0 arity tokens prior has wrong values at step = %i" % (step))

                prog_1_arity_non0 = prior[1][my_lib.get_choosable_prop("arity") != 0]
                works_bool = (prog_1_arity_non0 == prog_1_arity_non0_true[step]).all()
                self.assertTrue(works_bool,
                                msg="prog_1_arity_non0    tokens prior has wrong values at step = %i" % (step))
    ...
    def test_RelationshipConstraintPrior(self):
        ...
        # -------------------- CREATION --------------------
        try:
            my_prior = Prior.RelationshipConstraintPrior(
                library=my_lib,
                programs=my_programs,
        ...
        with self.assertRaises(AssertionError, msg="relationship arg must be valid"):
            my_prior = Prior.RelationshipConstraintPrior(library=my_lib,
                                                         programs=my_programs,
                                                         effectors=["n2", "sin"],
        ...
        expected_forbidden_tokens = [
            ...
            ["cos", "sin"
            ],  # 2 cos ancestors but tolerance is 1 for cos -> forbidding it, tolerance for sin is 0 -> forbidding it
            ["cos", "sin"
            ],  # 3 exp ancestors but tolerance is 2 for sin -> forbidding it, tolerance for cos is 0 -> forbidding it
    ...
    def test_NoUselessInversePrior(self):
        ...
        # -------------------- CREATION --------------------
        try:
            my_prior = Prior.NoUselessInversePrior(
                library=my_lib,
                programs=my_programs,
            )
        except:
        ...
        test_progs_str = np.array([
            ...
            [
                ...
                "n2",
                "sqrt",
            ],  # Previously violated prior
            [
                "add",
                "cos",
                "add",
                "n2",
            ],  # Testing f -> f-1 and f-1 -> f
            [
                "add",
                "sub",
                "sin",
    ...
    def test_NestedFunctions(self):
        ...
        # -------------------- CREATION --------------------
        try:
            my_prior = Prior.NestedFunctions(library=my_lib,
                                             programs=my_programs,
                                             functions=[
                                                 "cos",
                                                 "sin",
        ...
        with self.assertRaises(AssertionError, msg="functions arg must be valid"):
            my_prior = Prior.NestedFunctions(
                library=my_lib,
                programs=my_programs,
                functions="n2",
                max_nesting=1,
    ...
    def test_NestedTrigonometryPrior(self):
        ...
        # Assertion test : max_nesting
        with self.assertRaises(AssertionError, msg="max_nesting arg must be valid"):
            my_prior = Prior.NestedTrigonometryPrior(
                library=my_lib,
                programs=my_programs,
                max_nesting=-1,
            )
    ...
    def test_OccurencesPrior(self):
        ...
        # -------------------- CREATION --------------------
        try:
            my_prior = Prior.OccurrencesPrior(
                library=my_lib,
                programs=my_programs,
                targets=["add", "x"],
                max=[1, 2],
    ...
    def test_PhysicalUnitsPrior(self):
        ...
        my_lib = Lib.Library(args_make_tokens=args_make_tokens, superparent_units=[2, -2, 1], superparent_name="y")
        ...
        expected_allowed = [
            ...
            [
                ...
                ['mul', 'add', 'neg', 'n2', 'inv', 'cos', 'g', 'c', 'm', 'E0', 'z', 'x', 'v',
                 't'],  # -> not physical anymore, all tokens are allowed
            ],
            # Prog 1
            [
                ['mul', 'add', 'neg', 'n2', 'inv', 'E0'],  # step 0
            ...
            [
                ...
                ['mul', 'add', 'neg', 'n2', 'inv', 'cos', 'g', 'c', 'm', 'E0', 'z', 'x', 'v',
                 't'],  # -> not physical anymore, all tokens are allowed
                ['mul', 'add', 'neg', 'n2', 'inv', 'cos', 'g', 'c', 'm', 'E0', 'z', 'x', 'v',
                 't'],  # -> prog complete, all tokens are allowed
            ],
        ]
        ...
        my_programs = Prog.VectPrograms(batch_size=test_programs_idx.shape[0],
                                        max_time_step=test_programs_idx.shape[1],
                                        library=my_lib)
        my_prior = Prior.PhysicalUnitsPrior(
            library=my_lib,
            programs=my_programs,
        )
    ...
    def test_PriorCollection(self):
        ...
        with self.assertRaises(AssertionError,):
            my_collection = Prior.make_PriorCollection(
                ...
                programs=my_programs,
                priors_config=[
                    ("A_prior_that_does_not_exist", None),
                    ("HardLengthPrior", {
                        "min_length": min_length,
                        "max_length": max_length
                    }),
        ...
        # ------- TEST CREATION -------
        my_prior_HardLength = Prior.HardLengthPrior(library=my_lib,
                                                    programs=my_programs,
                                                    min_length=min_length,
                                                    max_length=max_length)
        ...
        priors_config = [("UniformArityPrior", None), ("SoftLengthPrior", {
            "length_loc": 10,
            "scale": 5,
        }), ("SoftLengthPrior", {
            "length_loc": 11,
