...
class FreeConstantsTable:
    ...
    def __repr__(self):
        s = "FreeConstantsTable for %s : %s" % (
            self.library.free_constants_tokens,
            self.shape,
        )
        return s
...
OPTIMIZERS = {"LBFGS": LBFGS_optimizer}

OPTIMIZERS_DEFAULT_ARGS = {"LBFGS": DEFAULT_LBFGS_OPTI_ARGS}
...
def optimize_free_const(func, params, y_target, loss="MSE", method="LBFGS", method_args=None):
    """
    ...
    if method_args is None:
        err_msg = "Optimizer args should be given or defined in free_const.OPTIMIZERS_DEFAULT_ARGS: %s" % (
