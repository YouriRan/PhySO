...
def assign_required_units_at_step(programs, step=None, from_scratch=False):
    ...
    if from_scratch:
        for i in range(step):
            ...
            tokens_cases_record = assign_required_units(
                programs=programs,
                coords=coords,
            )  # (batch_size,)
    else:
...
def assign_required_units(
    ...
    phy_units = np.full(shape=(n_tokens, Tok.UNITS_VECTOR_SIZE), fill_value=np.NAN,
    ...
    def apply_no_constraints(mask_case):
        apply_const_constraints(mask_case, const_is_constraining=False, const_phy_units=np.NAN)
    ...
    def get_parent_info():
        ...
        _, parent_is_constraining = programs.get_property_of_relative(
            coords,
            relative="parent",  # (n_tokens,)
            attr="is_constraining_phy_units")
        # physical units of the parent
        _, parent_phy_units = programs.get_property_of_relative(
            coords,
            relative="parent",  # (n_tokens,)
            attr="phy_units")
        # behavior id of the parent
        _, parent_behavior_id = programs.get_property_of_relative(
            coords,
            relative="parent",  # (n_tokens,)
            attr="behavior_id")
        # power of the parent
        _, parent_power = programs.get_property_of_relative(
            coords,
            relative="parent",  # (n_tokens,)
            attr="power")
        # children pos of the parent
        _, parent_children_pos = programs.get_property_of_relative(
            coords,
            relative="parent",  # (n_tokens,)
            attr="children_pos")
    ...
    def get_sibling_info():
        ...
        _, sibling_is_constraining = programs.get_property_of_relative(
            coords,
            relative="siblings",  # (n_tokens,)
            attr="is_constraining_phy_units")
        # physical units of the sibling
        _, sibling_phy_units = programs.get_property_of_relative(
            coords,
            relative="siblings",  # (n_tokens,)
            attr="phy_units")
        # behavior id of the sibling
        _, sibling_behavior_id = programs.get_property_of_relative(
            coords,
            relative="siblings",  # (n_tokens,)
            attr="behavior_id")
        # Is it a dummy ?
    ...
    # mask : does token fall into this case ?
    mask_case = np.logical_and.reduce((  # (n_tokens,)
        (tokens_cases_record == no_case_code), (pos == 0)))
    # Number of tokens falling into this case
    n_in_case = mask_case.sum()
    # Declaring tokens that fell into this case in
    tokens_cases_record[mask_case] = case_code  # (n_in_case,)
    ...
    programs.tokens.is_constraining_phy_units[mask_assign] = is_constraining[curr_token_already_constraining ==
                                                                             False]  # (n_tokens_already_constraining,)
    programs.tokens.phy_units[mask_assign] = phy_units[curr_token_already_constraining ==
                                                       False]  # (n_tokens_already_constraining, UNITS_VECTOR_SIZE)
